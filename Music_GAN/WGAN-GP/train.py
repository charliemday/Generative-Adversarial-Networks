...
def gradient_penalty_loss(y_true, y_pred, averaged_samples, gp_weight):
    # 1. Get the gradients
    gradients = backend.gradients(y_pred, averaged_samples)
    # 2. Compute the Euclidean Norm by squaring
    gradients_sqr = backend.square(gradients)
    # 3. Sum over rows
    gradients_sqr_sum = backend.sum(
        gradients_sqr, axis=np.arange(1, len(gradients_sqr)))
    # 4. Sqrt
    gradient_l2_norm = backend.sqrt(gradients_sqr_sum)
    # 5. Compute lambda * (1 - ||grad||)^2 still for each single sample
    gradient_penalty = gp_weight * backend.square(1 - gradient_l2_norm)
...
class Models():
    ...
    def __init__(self, wasserstein_loss, gradient_penalty_loss, avg_samples):
        self.w_loss = wasserstein_loss
        self.avg_samples = avg_samples
        # See lambda variable in WGAN-GP paper (https://arxiv.org/pdf/1704.00028.pdf)
        self.gp_weight = 10
        self.partial_gp = partial(
            gradient_penalty_loss, averaged_samples=self.avg_samples, gp_weight=self.gp_weight)
    ...
    def critic(self, in_shape=(28, 28, 1)):
        ...
        model.add(Conv2D(64, (4, 4), strides=(2, 2), padding='same',
                         kernel_initializer=init, kernel_constraint=const, input_shape=in_shape))
...
def train(self, epochs, batch_size=64, sample_interval=50, n_critic=5):
    ...
    for epoch in range(n_steps):
        ...
        x_latent, y_latent = self.generate_latent_points(
            self.latent_dim, batch_size)
...
models = Models(wasserstein_loss, averaged_samples)
